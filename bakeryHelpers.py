...
def parseMasses(
    ...
    if type(masses) not in [list, tuple] or len(masses) < 2:
        ...
        print(
            "[bakeryHelpers.py] Error: masses need to be given as e.g. %s (you will need to put it under parentheses)"
            % mdefault
        )
        sys.exit()
    lists = []
    for ctr, mtuple in enumerate(masses):  ## tuple by tuple
        ...
        if type(mtuple) in [str]:  ## descriptive strings
            ...
            # elif any(f"M0+{i}" in mtuple for i in range(5, 50, 1)) and ctr == 2:
                ...
                print(
                    "[bakeryHelpers.py] error: i know only 'half' or 'same' for a string, and only in middle position. the only exception is 'M0+nb' in the third position when we have 4 mass tuples with nb in range(5,50,1) and M0 is the mass of the LSP, this can't be used with 'half' though"
                )
                sys.exit()
    ...
    if len(lists[1]) == 0:
        print(
            "[bakeryHelpers] no daughter masses found. you sure you specified a meaningful mass array?"
        )
        sys.exit(-1)
    if lists[1][0] == "half":
        for x in lists[0]:
        ...
        for x in lists[0]:
            expr = lists[1][0].replace("half", f"{0.5 * x}")
            if len(lists) == 2:
                y = int(eval(expr))
                ret.append((int(x), y))
            else:
        ...
        lists[1][0] == "same"
        and any(f"M0+{i}" in lists[2][0] for i in range(5, 101, 1))
        and len(lists) == 4
    ):
        ...
        type(lists[1][0]) == str
        and any(f"M0+{i}" in lists[1][0] for i in range(5, 101, 1))
        and len(lists) == 3
    ):
                ...
                for k in range(len(lists[3])):
                    ret.append(
                        (
                            int(lists[0][x]),
                            int(lists[1][y]),
                            int(lists[3][k]) + int(substrings[1]),
                            int(lists[3][k]),
                        )
                    )
...
def filterForGap(masses, gap, isMin=True, indices=[0, 1]):
    ...
    if gap == None:
        return masses
    if len(masses) == 0:
        print("[bakeryHelpers] empty mass list, check your constraints on the masses!")
        sys.exit(-1)
...
def listAnalyses(cutlang: bool, checkmate: bool, colliderbit: bool):
...
def listAnalysesMA5():
    ...
    dn = [
        "ma5/tools/PAD/Build/SampleAnalyzer/User/Analyzer/",
...
def writeEmbaked(effs: dict, effi_file: PathLike, masses, recaster: str):
    ...
    try:
        ...
        f = open(effi_file, "wt")
        f.write(
            f"# EM-Baked {time.asctime()}. {npoints} points, {nregions} signal regions, checkmate2(direct)\n"
        )
        f.write("{")
    ...
    while ctr < 5:
        try:
            ...
            if not (proc.returncode == 0):
                print(
                    f"[bakeryHelpers] Executed process: \n{scmd}\n\nin"
                    f" directory:\n{directory}\n\nproduced an error\n\n"
            ...
            print(
                "[bakeryHelpers] ran into blocking io error. wait a bit then try again."
            )
...
def checkDelphesInstall(
    installdir: PathLike = "delphes", autocompile: bool = True
) -> bool:
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(
        description="show the masses for a given mass string"
    )
    argparser.add_argument("-m", "--masses", help="masses", type=str, default=None)
    argparser.add_argument(
        "--maxgap2",
        help="maximum mass gap between second and third, to force offshell [None]",
        type=float,
        default=None,
    )
    argparser.add_argument(
        "--mingap1",
        help="minimum mass gap between first and second, to force onshell or a mass hierarchy [None]",
        type=float,
        default=None,
    )
    argparser.add_argument(
        "--mingap2",
        help="minimum mass gap between second and third, to force onshell or a mass hierarchy [None]",
        type=float,
        default=None,
    )
    argparser.add_argument(
        "--mingap13",
        help="minimum mass gap between first and third, to force onshell or a mass hierarchy [None]",
        type=float,
        default=None,
    )
    argparser.add_argument(
        "--maxgap13",
        help="maximum mass gap between first and third, to force offshell [None]",
        type=float,
        default=None,
    )
    argparser.add_argument(
        "--maxgap1",
        help="maximum mass gap between first and second, to force offshell [None]",
        type=float,
        default=None,
    )
    ...
    masses = parseMasses(
        args.masses,
        mingap1=args.mingap1,
        maxgap1=args.maxgap1,
        mingap2=args.mingap2,
        maxgap2=args.maxgap2,
        mingap13=args.mingap13,
